...
void Skelcollapse::initParameters(Document* , RichParameterSet* , StarlabDrawArea* ){}
void Skelcollapse::algorithm(RichParameterSet*){
    ...
    ColorizeHelper(mesh).vscalar_to_vcolor("v:angle");   
    
    /// Change name and path of the file
    QFileInfo fi(mesh->path);
    QString basename = fi.baseName();
    QString currpath = fi.dir().path();
    mesh->path = currpath+"/"+basename+"_poles.off";
    qDebug() << "Remember to save the file!!"; 
}
