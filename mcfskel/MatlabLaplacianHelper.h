...
class MatlabCotangentLaplacianHelper : public CotangentLaplacianHelper, private MatlabEngineHelper{    
    IndexVertexProperty vindex;
    
public:
    MatlabCotangentLaplacianHelper(SurfaceMeshModel* mesh) : SurfaceMeshHelper(mesh), LaplacianHelper(mesh), CotangentLaplacianHelper(mesh){}
    ...
    void solve(){
        ...
        eval("OMEGA_L = spdiags(omega_L, 0, nv, nv);");
        eval("L = OMEGA_L*L;");
        /// Create bi-laplacian (which symmetrizes matrix if wasn't so)
        eval("L = L'*L;");
        /// Add constraints on LHS and RHS
        eval("P = spdiags(omega_H, 0, nv, nv);");
        eval("b = P*x0;");
